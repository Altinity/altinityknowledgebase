---
title: "Useful queries"
linkTitle: "Useful queries"
description: >
    Useful queries
---

* [Number of active parts in a partition](altinity-kb-number-of-active-parts-in-a-partition.md)
* [How to test different compression codecs.](../altinity-kb-schema-design/codecs/altinity-kb-how-to-test-different-compression-codecs.md)
* [Database Size - Table - Column size](altinity-kb-database-size-table-column-size.md)
* [Can detached parts be dropped?](detached-parts.md)
* [Datasets](altinity-kb-datasets.md)



