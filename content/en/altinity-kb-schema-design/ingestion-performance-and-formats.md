...
-- we test only formats parsing performace.
...
-- another option is to disable parallelizm `--input_format_parallel_parsing=0`
...
As you can see the JSONEachRow is the worst format  \(user 0m30.235s\) for this syntetic dataset. Native is the best \(user 0m1.945s\). TSV / CSV are good in wall time but spend a lot of CPU \(user time\).
